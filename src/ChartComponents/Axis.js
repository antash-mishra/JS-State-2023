...
const AxisBottom = props => {
  ...
  return (
    <g className="axis" transform={`translate(0, ${props.innerHeight})`}>
      ...
      {props.label &&
        <text className="axis-label" textAnchor="middle" transform={`translate(${props.innerWidth / 2}, 45)`}>
          {props.label}
        </text>
      }
    </g>
...
const AxisBandLeft = props => {
  return (
    <g className="axis">
      <line x1={0} y1={props.innerHeight} x2={0} y2={0} />
      {props.ticks.map(tick => (
        <g key={tick} transform={`translate(-3, ${props.scale(tick)})`}>
          <text
            key={tick}
            x={-10}
            y={0}
            textAnchor="end"
            alignmentBaseline="middle"
            transform={`translate(8, ${props.scale.bandwidth() / 2})`}
          >
            {tick}
          </text>
        </g>
